...
- `src/popular-picks/` — public, unauthenticated rankings of the most-collected albums and stickers across *all* users, for the home page. `GET /popular-picks/albums` and `GET /popular-picks/stickers` (no guard, unlike `collection`'s `/me/...` routes — this is global data, not personal). Built via `openspec` (`openspec/changes/add-popular-picks-home/`, or `openspec/specs/popular-picks/` once archived — see its `spec.md` for the exact ranking/tie-break/empty-state contract). `PopularPicksService` computes counts with `prisma.collectedSticker.groupBy({ by: ['stickerId'], _count: true })`, then walks the in-memory album catalog (not the grouped rows) to roll sticker counts up into album totals and to get the catalog-order tiebreak "for free" from a stable sort — there is no `albumId` column on `CollectedSticker`, so this cross-referencing happens in application code, not SQL. Top 5 each, sorted by count descending.

## Status

NestJS 11, TypeScript, ESLint, Jest/Supertest, `class-validator`/`class-transformer`, Prisma 7 + SQLite. Has the `albums`, `auth`, `collection`, and `popular-picks` feature modules (see above) alongside the default placeholder `AppController`/`AppService`. Update this file as the data model, endpoints, and persistence layer evolve.
