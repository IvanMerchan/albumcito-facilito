...
Non-trivial changes are planned with **OpenSpec** (`openspec/` — `opsx:propose` → `opsx:apply` → `opsx:archive`) before implementation: a proposal, a delta spec per capability under `openspec/changes/<name>/specs/`, a design doc, and a task checklist. Archived changes' specs move to `openspec/specs/<capability>/`, which becomes the durable source of truth for that capability's behavior going forward.

## Status

Frontend and backend have four features end-to-end: browsing the album catalog (and opening an album to see its stickers), email/password signup and login, a mandatory onboarding step that has every new signup add a first sticker to their personal collection before reaching their dashboard, and a home page that surfaces popular albums/stickers ranked by real collection data.

- Backend: `albums` feature module (`GET /albums`, `GET /albums/:albumId`, `GET /albums/:albumId/stickers`, in-memory seed), `auth` feature module (`POST /auth/signup`, `POST /auth/login`, `GET /auth/me`, JWT-protected), `collection` feature module (`POST /me/stickers`, `GET /me/stickers`, JWT-protected), and `popular-picks` feature module (`GET /popular-picks/albums`, `GET /popular-picks/stickers`, public — the first module planned via OpenSpec). `auth` and `collection` are backed by **Prisma + SQLite** — the first real persistence in this repo (see `apis/albumcito-facilito-api/CLAUDE.md`'s "Persistence" section for the details, including a couple of Prisma-7-specific gotchas). `albums` stays in-memory on purpose (fixed catalog content, not user data); `popular-picks` reads from both. Runs on port 3001.
- Frontend: home page (`/`) shows popular albums/stickers (when there's data) above the full album catalog; `/albums/[albumId]` shows an album's stickers; `/signup` and `/login` authenticate against the backend, with `signup` redirecting to `/onboarding` (not the dashboard) and `login` going straight to `/dashboard/[username]`. `/onboarding` → `/onboarding/[albumId]` lets the new user pick an album and a sticker; `/dashboard/[username]` then shows a summary of their collection. The session is a JWT in an httpOnly cookie, set by a Server Action — no browser-side call ever touches the API directly, so no CORS is needed.
